...
static void on_calculate_1rm(GtkButton *button, gpointer user_data) {
    ...
    if (!parse_integer_entry(tools->entry_1rm_weight, &weight) || !parse_integer_entry(tools->entry_1rm_reps, &reps) || reps <= 0 || reps >= 37) {
        set_label_text(tools->lbl_1rm_epley, "Błąd");
        set_label_text(tools->lbl_1rm_brzycki, "Błąd");
        return;
    }
...
static void on_calculate_pace(GtkButton *button, gpointer user_data) {
    ToolWidgets *tools = (ToolWidgets *)user_data;
    double time_min = 0.0;
    double distance_km = 0.0;
    if (!parse_double_entry(tools->entry_pace_time, &time_min) || !parse_double_entry(tools->entry_pace_distance, &distance_km)) {
        set_label_text(tools->lbl_pace_result, "Błąd");
        return;
    }
    if (distance_km <= 0.0) {
        set_label_text(tools->lbl_pace_result, "Błąd");
...
static void on_calculate_bmi(GtkButton *button, gpointer user_data) {
    ToolWidgets *tools = (ToolWidgets *)user_data;
    ...
    if (!parse_double_entry(tools->entry_bmi_height, &height_cm) || !parse_double_entry(tools->entry_bmi_weight, &weight_kg) || !parse_integer_entry(tools->entry_bmi_age, &age) || age <= 0) {
        set_label_text(tools->lbl_bmi_result, "Błąd");
        set_label_text(tools->lbl_bmr_result, "Błąd");
        return;
    }
    ...
    double bmr = 10.0 * weight_kg + 6.25 * height_cm - 5.0 * age + 5.0; // przyjęto wzór dla mężczyzny
    char text[64];
...
static void on_calculate_hr(GtkButton *button, gpointer user_data) {
    ToolWidgets *tools = (ToolWidgets *)user_data;
    int age = 0;
    if (!parse_integer_entry(tools->entry_hr_age, &age) || age <= 0 || age >= 120) {
        set_label_text(tools->lbl_hr_z1, "Błąd");
        set_label_text(tools->lbl_hr_z2, "Błąd");
        set_label_text(tools->lbl_hr_z3, "Błąd");
        set_label_text(tools->lbl_hr_z4, "Błąd");
        set_label_text(tools->lbl_hr_z5, "Błąd");
        return;
    }
    int max_hr = 220 - age;
...
static void connect_tool_callbacks(GtkBuilder *builder, GtkWidget *window) {
    ...
    GtkWidget *btn_1rm = GTK_WIDGET(gtk_builder_get_object(builder, "btn_calc_1rm"));
    GtkWidget *btn_pace = GTK_WIDGET(gtk_builder_get_object(builder, "btn_calc_pace"));
    GtkWidget *btn_bmi = GTK_WIDGET(gtk_builder_get_object(builder, "btn_calc_bmi"));
    GtkWidget *btn_hr = GTK_WIDGET(gtk_builder_get_object(builder, "btn_calc_hr"));

    if (btn_1rm)
        g_signal_connect(btn_1rm, "clicked", G_CALLBACK(on_calculate_1rm), tools);
    if (btn_pace)
        g_signal_connect(btn_pace, "clicked", G_CALLBACK(on_calculate_pace), tools);
    if (btn_bmi)
        g_signal_connect(btn_bmi, "clicked", G_CALLBACK(on_calculate_bmi), tools);
    if (btn_hr)
        g_signal_connect(btn_hr, "clicked", G_CALLBACK(on_calculate_hr), tools);
...
void show_dashboard_window(GtkApplication *app) {
    ...
    GtkLabel *user_name_label = GTK_LABEL(gtk_builder_get_object(builder, "dashboard_user_name"));
